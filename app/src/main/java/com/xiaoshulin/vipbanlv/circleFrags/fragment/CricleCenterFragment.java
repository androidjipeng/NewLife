 ...
 * 圈子中心
 */
public class CricleCenterFragment extends BaseMVPFragment implements IAllCircleView, View.OnClickListener {


    private AllCirclePresenter presenter;
    private List<CricleBean.DataBean> list = new ArrayList<>();
    ...
    public CricleCenterFragment(String cricleId) {
        // Required empty public constructor
        this.cricleId = cricleId;
    }

    public CricleCenterFragment() {
    }

    public static CricleCenterFragment newInstance(String cricleId) {
        CricleCenterFragment fragment = new CricleCenterFragment(cricleId);
        return fragment;
    }
    ...
    protected void initData() {
        ...
        presenter.getCircleIdData(cricleId);
    ...
    protected void initView() {
        content_recycle = mRootView.findViewById(R.id.circle_center_recycle);
//        content_recycle.isFocusable = false;
//        content_recycle.isFocusableInTouchMode = false;
        RecyclerView.LayoutManager manager = new LinearLayoutManager(getActivity());
        content_recycle.setLayoutManager(manager);

        adapter = new CircleAdapter1(getContext(), list,0);
...
//                         presenter.BuyPro(list.get(position).getCircleid(),"1");
    ...
    protected int getLayoutId() {
        return R.layout.fragment_cricle_center;
    }
    ...
    public void CircleListData(CricleBean bean) {
        ...
        if (list.size()>0)
        {
            ...
            try {
                InputStream is = getContext().getResources().getAssets().open(avatar + ".png");
                Bitmap bitmap = BitmapFactory.decodeStream(is);
                img.setImageBitmap(bitmap);
            } catch (IOException e) {
                e.printStackTrace();
            }
            tv_uid.setText("缘分值：" +  list.get(0).getName());
        }
...
//        EventBus.getDefault().post(eventBusMessage);
        ARouter.getInstance().build("/activity/CommonActivity")
                .withInt("tag",2)
